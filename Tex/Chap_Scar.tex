...
完全符合本征热化假说与完全违背本征热化假说是两个极端。很快介于两个极端的弱违背本征热化假说系统被发现。典型的例子就是量子多体伤痕。这一现象最早发现于冷原子中里德堡原子平台\cite{bernien2017probing}。在一维原子链中，每个格点上里德堡原子近似为二能级体系，原子间的库伦相互作用禁止相邻的两个原子处于二能级中的里德堡激发态，可以用PXP哈密顿量来描述。该实验在$|Z_2\rangle$初态出发的动力学中发现了局域算符反常的长时间震荡行为，并且其时间平均值并不等于吉布斯系综所预测的热力学平均值。
...
\bicaption{图（a）为沿$J=h/2$特殊比例下局域算符$\hat{L}=\hat{\sigma}_1^z\hat{\sigma}_2^z$时间平均（圆点）与系综平均（方点），其中蓝色点为$L=14$，橘色点为$L=18$，绿色点为有限尺寸修正结果，带有线性拟合的误差。图（b）为$J=0.3$（蓝色），$J=0.6$（橘色），$J=2.5$（绿色）下的有限尺寸修正，圆点代表时间平均，方点代表吉布斯系综平均。}{Fig (a) for time average(dot) and conanical ensemble average(square) of local operator $\hat{L}=\hat{\sigma}_1^z\hat{\sigma}_2^z$, blue points for $L=14$, orange points for $L=18$ and green points for result of finite size scaling with error bar of linear fitting. Fig (b) for finite size scaling of time average(dot) and Gibbs ensemble average(square) under $J=0.3$(blue), $J=0.6$(orange), $J=2.5$(green).}
\label{scar1_2}
\end{figure}
%***********************************
...
\bicaption{不同参数下的本征态到初态投影、半链纠缠熵、演化动力学。其中（a，d）对应$J=0.3$，（b,e）对应$J=0.6$，（c，f）对应$J=2.5$。橘黄色虚线代表有限尺寸修正后的$L=16$下的热力学平均。}{Overlap between initail state and eigenstate, entanglement of half chain and dynamics under different $J$ with $J:h=1/2$ fixed. Fig (a,d) for $J=0.3$, (b,e) for $J=0.6$, (d,f) for $J=2.5$. Orange dashed line for thermal average of $L=16$ after finite size scaling.}
...
\bicaption{图（a）为$h=0$下$\hat{L}=\hat{\sigma}_1^z\hat{\sigma}_2^z$的时间平均与热力学平均。图（b）为$h=4$下的时间平均与热力学平均。圆点代表有限尺寸修正下的时间平均，方点代表有限尺寸修正下的热力学平均。图（c）为$h=0,J=1$的动力学，图（d）为$h=5.4,J=2.5$下的动力学。图（b，c）中橘黄色虚线代表$J=h/2$，图（d）橘黄色虚线代表有限尺寸修正后的$L=16$下的热力学平均。  }{ Fig (a) for time average and Gibbs ensemble average of local operator under $h=0$. Fig (b) for time average and Gibbs ensemble average under $h=4$. Dots for time average and square point for Gibbs ensemble average after finite size scaling. Fig (c) for difference between time average and thermal Gibbs average. Orange dashed line in Fig (b,c) for $H=h/2$. Orange dashed line in Fig (d) for thermal average after finite size scaling.}
\label{h04}
\end{figure}
%***********************************
...
\bicaption{图（a）为$J=2.3,h=1.1$下初态为$|Z_2\rangle$的动力学演化。可以看到局域算符期望值长时间震荡的行为。图（b）为第一个格点的纠缠熵随时间的变化。我们选取此时链长为$L=16$。橘黄色虚线代表有限尺寸修正后的$L=16$下的热力学平均。  }{Fig(a) for dynamics of local operator $\hat{L}=\hat{\sigma}_1^z\hat{\sigma}_2^z$ starting from$|Z_2\rangle$. Fig(b) for entanglement entropy of spin degree on first site. We choose $L=16$ for simulation. Orange dashed line for thermal average of $L=16$ after finite size scaling.}
\label{weak}
\end{figure}
%***********************************
...
\bicaption{图（a）为$J=0.9,h=0.8$下初态为$|Z_2\rangle$的动力学演化。此时仍然可以看到局域算符期望值长时间震荡的行为。图（b）为第一个格点的约化密度矩阵随时间的变化。图（c）为$J=,h=$下初态为$|Y-\rangle$的动力学演化。此时仍然可以看到局域算符期望值长时间震荡的行为。图（d）为第一个格点的约化密度矩阵随时间的变化，初态为$|Y-\rangle$。我们选取此时链长为$L=16$。 橘黄色虚线代表有限尺寸修正后的$L=16$下的热力学平均。}{Fig(a) for dynamics of local operator $\hat{L}=\hat{\sigma}_1^z\hat{\sigma}_2^z$ under 为$J=0.9,h=0.8$. Fig(b) for entanglement entropy of spin degree on first site. Fig(c) for dynamics of local operator $\hat{L}=\hat{\sigma}_1^z\hat{\sigma}_2^z$. Fig(d) for entanglement entropy of spin degree on first site. We choose $L=16$ for simulation. Orange dashed line for thermal average of $L=16$ after finite size scaling.}
\label{midweak}
\end{figure}
%***********************************
